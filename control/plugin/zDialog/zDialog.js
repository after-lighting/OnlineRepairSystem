var IMAGESPATH = '/control/plugin/zDialog/'; //图片路径配置，如在有iframe情况下使用，此路径为最顶层窗口页面的相对路径
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" style="font-size:12px; line-height:1.4;border-collapse: collapse;">\
            ...
            <td valign="top" height="50"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#eaece9 url({IMAGESPATH}dialog_bg.jpg) no-repeat scroll right top;" id="_MessageTable_{thisID}">\
                <tr>\
                  ...
                  <td align="left" style="line-height: 16px;"><div id="_MessageTitle_{thisID}" style="font-weight:bold">' + this.MessageTitle + '</div>\
                    <div id="_Message_{thisID}" style="margin-top:2px">' + this.Message + '</div></td>\
                </tr>\
              </table></td>\
          </tr>\
          <tr>\
	...
	' + (function (obj) {
        ...
        if (obj.URL) return '<iframe width="100%" height="100%" frameborder="0" onload="CloseZDialogLoading()" style="border:none 0;" id="_DialogFrame_' + obj.ID + '" ' + (obj.InnerFrameName?'name="'+obj.InnerFrameName+'"':'')+' src="' + obj.displacePath() + '"></iframe>';
        return "";
    })(this) + '\
              </div></td>\
          </tr>\
          <tr id="_ButtonRow_{thisID}" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_{thisID}" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="BigButtonA" value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}"/>\
                <input type="button" class="BigButtonCloseA" value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
if(this.URL != null) html+='<div id="zDialog_Iframe_Loading" name="zDialog_Iframe_Loading" style="position:absolute;top:'+((this.Height/2+31)-20)+'px;left:'+((this.Width/2+10)-20)+'px"><img src="'+IMAGESPATH+'loading_big.gif" /></div>';
    ...
    if (this.URL) {
        ...
        if (!isGecko) {
            ...
            //this.innerFrame.onload = self.innerFrameOnload;//取消这个，否则在chrome下,iframe的onload不执行，不能关闭loading【2014-1-11】
        };
    };
...
function CloseZDialogLoading(){
   //document.getElementById("zDialog_Iframe_Loading").style.display = "none";
   //var oEle= document.all.zDialog_Iframe_Loading;
   var oEle = document.getElementsByName("zDialog_Iframe_Loading");
